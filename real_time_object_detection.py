...
def main():
	...
	num=10
	try:
		model = tf.keras.models.load_model('myproject/cafe_model.h5')
	...
	# construct the argument parse and parse the arguments
			...
			CLASSES = ["aeroplane", "background", "bicycle", "bird", "boat",
					   "mobile", "bus", "car", "person", "chair", "cow", "diningtable",
					   "person", "horse", "motorbike", "person", "mobile", "sheep",
					   "sofa", "train", "mobile"]

			# Assigning random colors to each of the classes
			COLORS = np.random.uniform(0, 255, size=(len(CLASSES), 3))
   # load our serialized model
			rsk=os.path.join(settings.BASE_DIR,'haarcascade_frontalface_default.xml')
			vsk=os.path.join(settings.BASE_DIR,'haarcascade_eye.xml')
			face_cascade = cv2.CascadeClassifier(rsk)
			eye_cascade = cv2.CascadeClassifier(vsk)
			...
			while True:
				...
				count = len(faces)
				if count <= 0:
					print("student is not looking at the screen / absent")
					error_list.append("student is not looking")

					num-=1
					engine= pyttsx3.init()
					engine.say('student is not looking at the screen')
					engine.runAndWait()
					return num
				...
				if count > 1:
					print("multiple faces detetcted")
					error_list.append("multiple faces detetcted")
					num -= 1
					engine=pyttsx3.init()
					engine.say('multiple faces detetcted')
					engine.runAndWait()
					return num
				...
				for i in np.arange(0, predictions.shape[2]):
					...
					if confidence > args["confidence"]:
						...
						if object_name == "mobile":
							print('mobile phone usage detected')
							error_list.append('mobile phone usage detected')
							# tts= gTTS(text='unusual avtivity detected',lang='en')
							# tts.save("output.mp3")
							# os.system("start output.mp3")
							engine=pyttsx3.init()
							engine.say('mobile phone detected. mobile phone detected')
							engine.runAndWait()
							num -= 1
							return num
